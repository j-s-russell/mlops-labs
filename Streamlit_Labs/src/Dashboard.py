...
FASTAPI_IRIS_MODEL_LOCATION = Path(__file__).resolve().parents[2] / 'FastAPI_Labs' / 'model' / 'housing_model.pkl'
...
def run():
    ...
    st.write("### Simple demo to predict house prices using FastAPI and Streamlit. Attach a JSON file with input_test field in the sidebar (like the example files) and predict the price of the house.")
            ...
            if FASTAPI_IRIS_MODEL_LOCATION.is_file():
                ...
                try:
                    ...
                    with st.spinner('Predicting...'):
                        ...
                        predict_housing_response = requests.post(f'{FASTAPI_BACKEND_ENDPOINT}/predict', client_input)
                    ...
                    if predict_housing_response.status_code == 200:
                        ...
                        if housing_content["response"]:
                            result_container.success(f"The predicted house value is: ${housing_content['response']*100000:.2f}")
                        else:
                        ...
                        st.toast(f':red[Status from server: {predict_housing_response.status_code}. Refresh page and check backend status]', icon="🔴")
